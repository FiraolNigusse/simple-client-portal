...
export function ChatWindow({ projectId, senderType }) {
  ...
  return (
    <Card className="flex flex-col h-[600px] p-0 overflow-hidden border-slate-800 bg-[#0B1220]/50 backdrop-blur-sm">
      ...
      <div 
        ref={scrollRef}
        className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-gradient-to-b from-transparent to-[#111827]/30"
      >
        {messages.map((msg, i) => {
              ...
              <div className={`max-w-[80%] space-y-1`}>
                <div className={`px-4 py-2.5 rounded-2xl text-sm shadow-sm ${
                  isMe 
                    ...
                    : "bg-surface text-portal-text border border-slate-800 rounded-tl-none"
                }`}>
                  {msg.content}
                </div>
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center space-y-2 opacity-40">
            ...
            <p className="text-sm font-medium">No messages yet. Send a hello!</p>
          </div>
        )}
      </div>

      <form onSubmit={handleSend} className="p-4 bg-surface border-t border-slate-800 flex gap-2">
        <input 
          type="text"
          className="flex-1 bg-background border border-slate-800 rounded-xl px-4 py-2 text-sm text-portal-text focus:outline-none focus:border-primary transition-all"
